...
| `owner` | 下一步负责的 accountable human / business owner | Security Policy Owner |
...
Review Appendix Exception Item 折叠态固定只展示这 5 个字段，不展示 `supporting_agent`。如果用户、评委或 reviewer 需要看 Agent 协助链路，应进入 Review Appendix Exception Detail。
...
`freshness_label` 只出现在 Review Appendix Exception Detail，不新增到折叠态 Review Appendix Exception Item。折叠态仍通过 `reason_or_blocker` 概括 stale / missing / conflicting 等影响。
...
- `owner` 必须是 accountable human / business owner。
- Agent 不能作为 `owner`，即使它发现了 blocker、提出了建议或执行了证据检索。
- Agent 可以在展开详情中作为 `supporting_agent` 出现，用于说明谁提供了证据、建议或 handoff，但不承担业务责任。
- 如果暂时无法确定 human / business owner，`owner` 应写成待分配的业务角色，例如 `Security Policy Owner needed`，而不是把 Agent 填进去。

Review Appendix Exception Owner 标准句式：

> Review Appendix Exception Owner: Security Policy Owner owns Q-006 next action; supporting_agent: evidence-retriever-agent provided the stale/conflicting evidence summary.
