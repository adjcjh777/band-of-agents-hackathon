...
|---|---|---|---|---|---|
| 2026-05-31 | `feature/trustroom-governed-evolution-spec` | Codex | Codex | T8 agent prompts and task envelopes completed | `uv run pytest tests/test_agent_prompts.py -v`; `rg -n "production deployment|legal advice|certification|fully compliant" src/trustroom/agents/prompts docs/agent-task-envelopes.md`; `rg -n "@mention|handoff|evidence|human approval|request_changes" src/trustroom/agents/prompts`; `uv run pytest -v`; `git diff --check` |
